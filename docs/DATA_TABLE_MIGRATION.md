...
- `TowerMaxLange`, `SplashRadius`, `ProjectileSize`, `MonsterAtkRange`는 Unity 기본 단위의 Collider Radius 값으로 입력한다. 런타임에서는 `1 Unity unit = 128px`로 일괄 변환한다. 예: 10 -> 1280px, 5 -> 640px, 0.25 -> 32px.
...
| 투사체 크기/관통/스플래시 | `TowerData.ProjectileSize`, `PiercingCount`, `SplashRadius` | `ProjectileSize`, `SplashRadius`는 Unity Collider Radius 단위로 입력하고 런타임에서 `x128px` 변환한다. 같은 `ProjectileID`를 공유해도 포탑별 override가 우선한다. |
