...
- `TowerMaxLange`, `SplashRadius`, `MonsterAtkRange`는 Unity 기본 단위의 Collider Radius 값으로 입력한다. 런타임에서는 `1 Unity unit = 128px`로 일괄 변환한다. 예: 10 -> 1280px, 5 -> 640px, 0.25 -> 32px.
- `ProjectileSize`는 탄의 전체 크기/지름으로 입력한다. 런타임 충돌 반지름은 `ProjectileSize * 128px / 2`로 변환한다. 예: 0.75 -> 지름 96px, 반지름 48px.
...
| `MonsterData` | `MonsterID`, `MonsterName`, `MonsterType`, `ExpTypeID`, `MonsterHp`, `MonsterAtk`, `MonsterAtkSpeed`, `MonsterAtkRange`, `MonsterMoveSpeed`, `MonsterSprite`, `Desc` | `monsters{}` + `defaultConfig` | 절대 스탯을 런타임 배율로 변환한다. `MonsterAtkRange`는 Unity Collider Radius 단위로 입력하고 런타임에서 `x128px` 변환한다. 경험치는 `ExpTypeID`로 읽는다. |
...
| `LevelData` | `LevelID`, `GoalLevel`, `RequiredXP`, `IsMaxLevel`, `Description`, `PerkEventType` | `levelData` | 해당 `GoalLevel`에 도달하기 위해 이전 레벨에서 필요한 경험치를 런타임 필요 경험치로 사용한다. |
...
4. 완료: `LevelData/ExpData`를 경험치와 레벨업 계산 원본으로 변경. `RequiredXP`는 해당 `GoalLevel`에 도달하기 위해 이전 레벨에서 필요한 경험치로 보고, 런타임은 이를 현재 레벨 기준 `xpCostByLevel`로 재매핑한다. 비어 있는 레벨 구간은 인접 행 사이를 자동 보간한다.
...
| 포탑 공격력/공속/사거리/탄약 | `TowerData.TowerAtk`, `TowerAtkSpeed`, `TowerMaxLange`, `TowerMaxAmmo` | 공격력/공속/탄약은 시트 값을 우선한다. `TowerMaxLange`는 Unity Collider Radius 단위로 입력하고 런타임에서 `x128px` 변환한다. |
| 투사체 크기/관통/스플래시 | `TowerData.ProjectileSize`, `PiercingCount`, `SplashRadius` | `ProjectileSize`는 탄 지름 단위로 입력해 런타임 반지름은 `x128px / 2`로 변환한다. `SplashRadius`는 Unity Collider Radius 단위로 입력하고 런타임에서 `x128px` 변환한다. 같은 `ProjectileID`를 공유해도 포탑별 override가 우선한다. |
