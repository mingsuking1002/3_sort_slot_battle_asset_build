...
- `TowerMaxLange`, `SplashRadius`, `MonsterAtkRange`는 Unity 기본 단위의 Collider Radius 값으로 입력하고 런타임에서 `1 Unity unit = 128px`로 일괄 변환한다.
- `ProjectileSize`는 탄의 전체 크기/지름으로 입력하고, 런타임 충돌 반지름은 `ProjectileSize * 128px / 2`로 변환한다.
