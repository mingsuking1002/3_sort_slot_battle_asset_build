...
const checks = [
  ...
  [files.bot.includes("mixedPairSetup") && files.bot.includes("clear-blocker") && files.bot.includes("blockerPieceKey"), "혼합 슬롯 페어 생성과 방해 기물 제거 계획"],
  ...
  [files.bot.includes("chooseBoardUnlockMove") && files.bot.includes("unlock = { attack: [], repair: [] }") && files.bot.includes("idleDecisionStreak") && files.bot.includes("boardUnlockCount"), "막힌 보드 응급 해제와 입력 중단 방지"],
